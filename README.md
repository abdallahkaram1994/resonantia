...
**Status:** milestones 1 (scaffold), 2 (films end to end) and 3 (games, albums and the background worker). You can load films, games and albums into one catalog and run jobs on a worker. Search and the page still return films only. Cross-type search and filters (milestone 4), the LLM layer, abuse protection and deployment come later. The plan and the decision log are in [docs/SPEC.md](docs/SPEC.md).
...
- **Source terms.** IGDB (games) is free for non-commercial use under the Twitch Developer Services Agreement, and Last.fm (album tags) is non-commercial only. Album summaries are from Wikipedia (CC BY-SA), and MusicBrainz core data is CC0. Attribution for these will appear in the footer when their data is shown (milestones 4 and 8). The decision log in the SPEC records what is still unchecked.
...
| Path | What |
|---|---|
| `backend/catalog/` | Items, the film, game and album sources, embedding adapter, ingest and embed commands, background jobs (`tasks.py`), search |
